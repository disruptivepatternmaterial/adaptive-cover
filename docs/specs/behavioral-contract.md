...
| Seasonal threshold hysteresis | Summer and winter are entered at their documented thresholds and left only after the reading clears them by `SEASON_TEMP_HYSTERESIS`. Inside the band the previously selected season holds, so sensor dither across a threshold cannot cycle covers between winter's fully-open target and summer's anti-glare geometry. The held season comes from the coordinator's last `control_method`; no remembered season evaluates every threshold at its nominal edge. |
| Predictive heat scope | Today's forecast high argues for summer only while that heat is still ahead: it is ignored once solar elevation is below `PREDICTIVE_HEAT_MIN_ELEVATION`, and when no elevation is supplied it is honored rather than inferred away. Measured heat is never gated on elevation — a room above `temp_high` reports summer at any sun position. |
| Tracked inputs | Every entity that steers a position — inside and **outside** temperature, presence, weather, cloud coverage, start/end time, window contacts, and `sun.sun` — is registered for state-change updates, deduplicated so one entity serving two roles refreshes once. An input read during an update but not tracked reacts only when some other tracked entity happens to change. |
